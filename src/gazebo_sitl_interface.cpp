...
#include "common.h"
#include "gazebo_sitl_interface.h"
...
GZ_REGISTER_MODEL_PLUGIN(GazeboSITLInterface);

GazeboSITLInterface::~GazeboSITLInterface() {
  event::Events::DisconnectWorldUpdateBegin(updateConnection_);
}

void GazeboSITLInterface::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  const char *env_lat = std::getenv("SITL_HOME_LAT");
  const char *env_lon = std::getenv("SITL_HOME_LON");
  const char *env_alt = std::getenv("SITL_HOME_ALT");
...
*/
  ...
  updateConnection_ = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&GazeboSITLInterface::OnUpdate, this, _1));

  // Subscriber to IMU sensor_msgs::Imu Message and SITL message
  imu_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + imu_sub_topic_, &GazeboSITLInterface::ImuCallback, this);
/*  lidar_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + lidar_sub_topic_, &GazeboSITLInterface::LidarCallback, this);
  opticalFlow_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + opticalFlow_sub_topic_, &GazeboSITLInterface::OpticalFlowCallback, this);
  sonar_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + sonar_sub_topic_, &GazeboSITLInterface::SonarCallback, this);*/
  ...
  gps_update_interval_ = 0.1;  // in seconds for 10Hz
  ...
  if (_sdf->HasElement("fc_addr")) {
    std::string mavlink_addr = _sdf->GetElement("fc_addr")->Get<std::string>();
    if (mavlink_addr != "INADDR_ANY") {
      mavlink_addr_ = inet_addr(mavlink_addr.c_str());
      if (mavlink_addr_ == INADDR_NONE) {
        fprintf(stderr, "invalid fc_addr \"%s\"\n", mavlink_addr.c_str());
        return;
      }
    }
  }
  if (_sdf->HasElement("fc_udp_port")) {
    fc_udp_port_ = _sdf->GetElement("fc_udp_port")->Get<int>();
  }

  if (_sdf->HasElement("recv_udp_port")) {
    recv_udp_port_ = _sdf->GetElement("recv_udp_port")->Get<int>();
  }
...
//  _myaddr.sin_port = htons(recv_udp_port_);
  ...
  _srcaddr.sin_port = htons(fc_udp_port_);
...
void GazeboSITLInterface::OnUpdate(const common::UpdateInfo& /*_info*/) {
...
//      gzerr << i << " : " << turning_velocities_msg.motor_speed(i) << " : " << input_reference_[i] << "\n";
    }
    ...
    // gzerr << 0 << " : " << turning_velocities_msg.motor_speed(0) << " : " << input_reference_[0] << "\n";
    motor_velocity_reference_pub_->Publish(turning_velocities_msg);
  }

  last_time_ = current_time;

  FDMout.timestamp = world_->GetSimTime().Double();
  ...
  // GPS data
  if (current_time.Double() - last_gps_time_.Double() > gps_update_interval_) {  // 10Hz
	FDMout.type |= 0x01 << 4; // Model velocity
	FDMout.velocityXYZ[0] = velocity_current_W.x;
	FDMout.velocityXYZ[1] = velocity_current_W.y;
	FDMout.velocityXYZ[2] = velocity_current_W.z;

	FDMout.type |= 0x01 << 5; // Model position
	FDMout.positionXYZ[0] = pos_W_I.x;
	FDMout.positionXYZ[1] = pos_W_I.y;
	FDMout.positionXYZ[2] = pos_W_I.z;
...
//    gps_msg.set_z(hil_gps_msg.alt / 1000.f);
    gps_msg.set_z(pos_W_I.z + alt_home);
...
void GazeboSITLInterface::send_message() {
  int len = sendto(_fd, &FDMout, sizeof(fdmPacket), 0, (struct sockaddr *)&_srcaddr, sizeof(_srcaddr));
  FDMout.type = 0;
  if (len <= 0) {
    printf("Failed sending fdm data\n");
  }
}

/*void GazeboSITLInterface::send_mavlink_message(const uint8_t msgid, const void *msg, uint8_t component_ID) {
  component_ID = 0;
...
void GazeboSITLInterface::ImuCallback(ImuPtr& imu_message) {
  ...
  */
  ...
  math::Quaternion q_nr = q_ng*q_gr;
  ...
  float declination = 0.0f;//get_mag_declination(lat_rad, lon_rad);
  ...
  math::Vector3 accel_b = q_br.RotateVector(accel_r);
  ...
  math::Vector3 gyro_b = q_br.RotateVector(gyro_r);
...
// gyro_b?
  FDMout.type |= 0x01 << 0; // IMU angular velocity
  FDMout.imuAngularVelocityRPY[0] = gyro_r.x;
  FDMout.imuAngularVelocityRPY[1] = gyro_r.y;
  FDMout.imuAngularVelocityRPY[2] = gyro_r.z;

// accel_b?
  FDMout.type |= 0x01 << 1; // IMU linear acceleration
  FDMout.imuLinearAccelerationXYZ[0] = accel_r.x;
  FDMout.imuLinearAccelerationXYZ[1] = accel_r.y;
  FDMout.imuLinearAccelerationXYZ[2] = accel_r.z;

// q_nb?
  FDMout.type |= 0x01 << 3; // IMU quaternion orientation
  FDMout.imuOrientationQuat[0] = q_gr.w;
  FDMout.imuOrientationQuat[1] = q_gr.x;
  FDMout.imuOrientationQuat[2] = q_gr.y;
  FDMout.imuOrientationQuat[3] = q_gr.z;

}

/*void GazeboSITLInterface::LidarCallback(LidarPtr& lidar_message) {
...
/*void GazeboSITLInterface::OpticalFlowCallback(OpticalFlowPtr& opticalFlow_message) {
...
/*void GazeboSITLInterface::SonarCallback(SonarSensPtr& sonar_message) {
  mavlink_distance_sensor_t sensor_msg;
...
/*ssize_t GazeboSITLInterface::receive(void *_buf, const size_t _size, uint32_t _timeoutMs)
{
  fd_set fds;
  struct timeval tv;
...
/*void GazeboSITLInterface::pollForMAVLinkMessages(double _dt, uint32_t _timeoutMs)
{
...
void GazeboSITLInterface::pollForPWM(double _dt, uint32_t _timeoutMs)
{
  // convert timeout in ms to timeval
  struct timeval tv;
  tv.tv_sec = _timeoutMs / 1000;
  ...
  if (fds[0].revents & POLLIN) {
    int len = recvfrom(_fd, &PWMin, sizeof(ServoPacket), 0, (struct sockaddr *)&_srcaddr, &_addrlen);
    if (len == sizeof(ServoPacket)) {
      // set rotor speeds, controller targets
      last_actuator_time_ = world_->GetSimTime();
      input_reference_.resize(n_out_max);
      for (int i = 0; i < input_reference_.size(); i++) {
        input_reference_[i] = (PWMin.motorSpeed[i] + input_offset_[i]) * input_scaling_[i] + zero_position_armed_[i];
//        if(joints_[i]) gzerr << i << " : " << PWMin.motorSpeed[i] << " : " << input_reference_[i] << "\n";
      }

      received_first_referenc_ = true;
    } else {
      gzerr << "received size error (" << len << ")\n";
    }
  }
}

/*void GazeboSITLInterface::handle_message(mavlink_message_t *msg)
{
  switch(msg->msgid) {
...
void GazeboSITLInterface::handle_control(double _dt)
{
    // set joint positions
    for (int i = 0; i < input_reference_.size(); i++) {
      if (joints_[i]) {
