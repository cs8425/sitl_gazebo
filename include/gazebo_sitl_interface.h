...
static const uint32_t kDefaultFCUdpPort = 9003;
static const uint32_t kDefaultRecvUdpPort = 9002;
...
typedef struct {
  /// \brief data field contain in packet
  uint16_t type;
...
class GazeboSITLInterface : public ModelPlugin {
 public:
  GazeboSITLInterface()
      : ModelPlugin(),
...
//        sonar_sub_topic_(kDefaultSonarTopic),
        ...
        lat_rad(0.0),
        lon_rad(0.0),
        fc_udp_port_(kDefaultFCUdpPort),
        recv_udp_port_(kDefaultRecvUdpPort)
        {}
  ~GazeboSITLInterface();
...
//  void pollForMAVLinkMessages(double _dt, uint32_t _timeoutMs);
  void pollForPWM(double _dt, uint32_t _timeoutMs);
